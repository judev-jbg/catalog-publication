...
Utilizado para notificar eventos en la publicación de catálogos.
"""
import asyncio
import aiohttp
import aiosmtplib
...
logger = logging.getLogger("CatalogPublicationLogger")
...
class NotificationManager:
    ...
    def __init__(self):
        ...
        self.slack_config = {
            ...
            "channel": os.getenv("SLACK_CHANNEL", "#catalog-publication"),
            "username": os.getenv("SLACK_USERNAME", "Catalog-Bot")
        }
    ...
    async def send_email_notification(self, subject: str, message: str, error_details: Optional[Dict[str, Any]] = None, is_critical: bool = False):
        ...
        try:
            ...
            email_msg["Subject"] = f"{priority_prefix} - Publicación de Catálogos: {subject}"
    ...
    async def send_critical_notification(self, title: str, message: str, error_details: Optional[Dict[str, Any]] = None):
        ...
        slack_task = self.send_slack_notification(
            f"{title}: {message}", error_details, is_critical=True)
    ...
    def _create_html_email_content(self, subject: str, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool) -> str:
        ...
        <body>
            <div class="header">
                <h2>{'🚨 Error' if is_critical else '⚠️ Advertencia'} - Publicación de Catálogos</h2>
            </div>
            <div class="content">
            ...
            <div class="footer">
                Publicación de Catálogos - Sistema Automatizado - Generado automáticamente
            </div>
        </body>
        </html>
        """
    ...
    def _create_plain_email_content(self, subject: str, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool) -> str:
        ...
        content = f"""
{priority} - Publicación de Catálogos
...
Publicación de Catálogos - Sistema Automatizado - Generado automáticamente
    ...
    def _create_slack_payload(self, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool, type: str = "info") -> Dict[str, Any]:
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": f"{emoji} {status_text} - Publicación de Catálogos",
                    "emoji": True
                }
            },
            {
        ...
        blocks.append({
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": f"Publicación de Catálogos - Sistema Automatizado | `Timestamp: {timestamp}`"
                }
            ]
        })
    ...
    async def notify_warning(self, title: str, message: str, error_details: Optional[Dict[str, Any]] = None):
        ...
        slack_result = await self.send_slack_notification(f"{title}: {message}", error_details, is_critical=False)
